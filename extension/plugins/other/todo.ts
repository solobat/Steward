...
export default function(Steward: StewardApp): Plugin {
  ...
  function handleDoneEnter(item, command, query, { shiftKey }) {
    if (item) {
      ...
      if (shiftKey) {
        ...
        return deleteDone(todo).then(res => {
          addTodo(res);
        });
      }
    }
      ...
      .then(() => {
        return getTodos().then(resp => {
          ...
          return browser.storage.sync
            ...
            .then(() => {
              Toast.success(`Add todo [${todoText}]`, 'TodoList', {
                timeOut: 1000,
              });
  ...
  function getDones() {
    return browser.storage.local
      .get(STORAGE.DONE)
      .then(results => results[STORAGE.DONE]);
  }

  function getTodos() {
    return browser.storage.sync
      .get(STORAGE.TODO)
      .then(results => results[STORAGE.TODO]);
  }
