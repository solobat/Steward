...
function handleWallpaperAction(action, command) {
  if (action.selector) {
    ...
    addToBlackList(localStorage.wallpaper)
      .then(() => {
...
// copy from https://github.com/suxiaogang/WeiboPicBed
function uploadToWeiBoPicBed(imgData): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open(
      'POST',
      'https://picupload.weibo.com/interface/pic_upload.php?ori=1&mime=image%2Fjpeg&data=base64&url=0&markpos=1&logo=&nick=0&marks=1&app=miniblog',
    );
    xhr.send(data);
  });
}
    ...
    return getDataURI(window.localStorage.wallpaper)
      ...
      .then(imgUrl => {
        if (imgUrl) {
          ...
          return saveWallpaper(imgUrl, command) as any;
        }
      })
      .catch(resp => {
