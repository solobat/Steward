...
export const filterByName = (suggestions, text) =>
  util.getMatches(suggestions, text, 'name');
export const filterByPath = (suggestions, text) =>
  util.getMatches(suggestions, text, 'path');
...
export function getShareFields(context, win) {
  const elems: any = Array.from(
    context.querySelectorAll('meta[property^="og:"]'),
  );
...
export class Website {
  ...
  async initActions(actions: any[] = []) {
    const results = actions.filter(action => {
      ...
      return patterns.some(pattern =>
        minimatch(this.pageMeta.pathname, pattern),
      );
    });
  ...
  generateShareUrls() {
    ...
    generateSocialUrls(shareInfo).then((links: any) => {
    ...
    if (first === TRIGGER_SYMBOL.PATH) {
      if (text === TRIGGER_SYMBOL.PATH) {
        ...
        return Promise.resolve(
          filterByPath(this.paths, text).map(mapTo('action')),
        );
      }
    } else if (first === TRIGGER_SYMBOL.OUTLINE) {
      return Promise.resolve(
        this.outline.filter(outlineNameFilter).map(mapTo('action', 'outline')),
      );
    } else if (first === TRIGGER_SYMBOL.ANCHOR) {
      return Promise.resolve(
        this.anchors.filter(anchorNameFilter).map(mapTo('action', 'anchor')),
      );
      ...
      return Promise.resolve(
        this.paths.filter(cnNameFilter).map(mapTo('action')),
      );
    }
  }
...
function getDefaultSiteInfo(meta) {
  return {
    ...
    paths: [],
    navs:
      'nav a,.nav a,.header-nav a,.topbar a,#topnav a,.nav-wrapper a,.topnav a',
    disabled: false,
    outlineScope:
      '.markdown-section,#js_article,.doc-article-inner,.theme-container .page .content',
    isDefault: true,
    anchors: [],
  };
}
  ...
  return helper.init().then(sites => {
    let mixedSites = sites.filter(site => {
      const isMatch = site.host.startsWith('http')
        ? minimatch(meta.baseURL, site.host)
        : host === site.host;
...
const helper = {
  ...
  create(info) {
    if (info.title && info.host) {
      ...
      return Promise.resolve(
        websiteList.chromeStorage.create(website).then(() => {
  ...
  remove(id) {
    ...
    return Promise.resolve(
      websiteList.chromeStorage.destroy(model).then(() => {
        return this.refresh();
      }),
    );
  },
