...
虽然没怎么推广，用户也不是很多，但一有空闲就会维护<em>[?]</em>，毕竟${extType}几乎已经成为我使用频率最高的扩展了。<br>
<br>
...
或通过下方的二维码 <span class="notice">请我喝杯咖啡</span><em>[${stat[version].pay}]</em>，这是对${extType}的最大鼓励。
虽然一直在安静地开发，但也希望通过大家的口口相传，让更多的人知道
<a href="https://chrome.google.com/webstore/detail/${storeId}">${extType}</a><em>[${stat[version].user}]</em>的存在。
<div class="qrcodes">
    <div>
        <h3>微信</h3>
...
<a href="https://chrome.google.com/webstore/detail/${storeId}/reviews'">praise</a><em>[${stat[version].comment}]</em>
 or <a href="https://github.com/solobat/Steward" target="_blank">star</a><em>[${stat[version].star}]</em>，
and you can ask me for a cup of coffee through the link below<em>[${stat[version].pay}]</em>.
...
<a href="https://chrome.google.com/webstore/detail/${storeId}">${extType}</a><em>[${stat[version].user}]</em>。
<br><br>
<div>
