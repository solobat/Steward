  ...
  load(ctx: LoadContext, filter?: string) {
    ...
    request<ExtItem[]>({ action: "getExtensions", data: { enabled: true, query: filter ?? "" } })
      .then((list) => {
        ...
        ctx.setItems(items.length ? items : [createStateItem("empty", { title: "No enabled extensions" })]);
        ctx.setSelectedIndex(0);
      })
      .catch(() => {
        ctx.setLoading(false);
        ctx.setItems([createStateItem("error", { title: "Failed to load enabled extensions" })]);
      });
  },
};
